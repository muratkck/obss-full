...
import { addSeller } from '../AuthService'; // AuthService içindeki addSeller metodunu import edin

const AddSellerForm = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      setLoading(true);
      try {
        await addSeller(formData); // POST isteği gönder
        alert("Seller is added succesfully!")
        onSuccess(); // Başarılı işlemlerden sonra üst bileşeni bilgilendir
        setFormData({
          username: '',
          password: '',
          name: '',
